...
[^1]: A few caveats here: although it should be single pass theoretically, in practice we use two phases—one for the spread of the accusation and a second to clean up any --- triangles involving the scapegoat to make them +--. After optimization, the algorithm now runs remarkably fast: a 5,000-node network completes in under 4 seconds—what would have taken nearly 4 hours with naive implementation. The primary performance gains came from (1) adjacency list caching eliminating O(E) neighbor lookups, (2) neighbor-based triangle finding reducing O(V) to O(degree) iterations during BFS, and (3) optimized balance checking using edge-based enumeration instead of O(V³) brute force. This 850x speedup at 1,000 nodes demonstrates that the mechanism's simplicity isn't just conceptual, it's computationally efficient in practice. See [OPTIMIZATION_RESULTS.md](../mimetic-contagion/docs/OPTIMIZATION_RESULTS.md) for more details.
